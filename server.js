...
async function checkTokenAndRole(req, res, roleList){
  ...
  if (token && token.startsWith('Bearer ')) {
    token = token.substring(7); // 'Bearer ' çıkart
    console.log(token)
  }
  ...
  console.log(token)
  if(!token){
    res.status(401).send({error: 'token yok : User is not authenticated'});
    return false;
  }
  try{
    ...
    if(blacklistedTokens.includes(token)){
      res.status(401).send({error: 'blaclist: User is not authenticated'});
    ...
    if(!existingUser){
      res.status(401).send({error: 'existingyok User is not authenticated'});
      return false;
    }
    if(roleList && roleList.length > 0 && !roleList.includes(existingUser.role)){
      res.status(403).send({error: 'rolelist hatası User is not authorized'});
      return false;
    }
  }
  catch(err){
    res.status(401).send({error: 'catch hata User is not authenticated'});
    return false;
  }
  return true;
}
